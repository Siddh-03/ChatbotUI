import React from "react";
import { Link, useLocation } from "react-router-dom";
import {
  FaUserShield,
  FaUsers,
  FaRobot,
  FaClipboardList,
  FaCreditCard,
  FaComments,
  FaSignOutAlt,
} from "react-icons/fa";
import { adminService } from "../../services/adminService";

const AdminSidebar = ({ collapsed, onToggle }) => {
  const location = useLocation();
  const isActive = (path) => (location.pathname === path ? "dash-active" : "");

  return (
    <aside className={`dash-sidebar ${collapsed ? "dash-collapsed" : ""}`}>
      <div className="dash-sidebar-header">
        <div className="dash-logo-container">
          <div className="dash-logo-icon">
            {/* You can use a different logo or the same one with a badge */}
            <FaUserShield
              style={{ fontSize: "1.8rem", color: "var(--dash-primary-color)" }}
            />
          </div>
          <h2 className="dash-logo-text">Admin Panel</h2>
        </div>
        <button className="dash-sidebar-toggle-btn" onClick={onToggle}>
          <i className="fas fa-bars"></i>
        </button>
      </div>

      <nav className="dash-main-nav">
        <ul>
          <li className={isActive("/admin/dashboard")}>
            <Link to="/admin/dashboard">
              <span className="dash-nav-icon">
                <FaClipboardList />
              </span>
              <span className="dash-nav-text">Overview</span>
            </Link>
          </li>
          <li className={isActive("/admin/users")}>
            <Link to="/admin/users">
              <span className="dash-nav-icon">
                <FaUsers />
              </span>
              <span className="dash-nav-text">User Management</span>
            </Link>
          </li>
          <li className={isActive("/admin/bots")}>
            <Link to="/admin/bots">
              <span className="dash-nav-icon">
                <FaRobot />
              </span>
              <span className="dash-nav-text">Bot Management</span>
            </Link>
          </li>
          <li className={isActive("/admin/plans")}>
            <Link to="/admin/plans">
              <span className="dash-nav-icon">
                <FaCreditCard />
              </span>
              <span className="dash-nav-text">Plans & Subs</span>
            </Link>
          </li>
          <li className={isActive("/admin/feedbacks")}>
            <Link to="/admin/feedbacks">
              <span className="dash-nav-icon">
                <FaComments />
              </span>
              <span className="dash-nav-text">Feedbacks</span>
            </Link>
          </li>
        </ul>
      </nav>

      <div className="dash-sidebar-footer">
        <button
          onClick={adminService.logout}
          className="dash-button-secondary"
          style={{ width: "100%", justifyContent: "center" }}
        >
          <span className="dash-nav-icon">
            <FaSignOutAlt />
          </span>
          {!collapsed && "Logout"}
        </button>
      </div>
    </aside>
  );
};

export default AdminSidebar;
